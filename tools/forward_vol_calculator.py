...
class ForwardVolApp(tk.Tk):
    ...
    def _build_ui(self):
        ...
        formula_text = (
            "Formula\n"
            "\n"
            "  s_fwd = sqrt( (s2^2*T2 - s1^2*T1) / (T2 - T1) )\n"
            "  FF = (s1 - s_fwd) / s_fwd\n"
            "\n"
            "Definitions\n"
            "  T = DTE / 365\n"
            "  s = IV / 100  (IV entered as a percent, e.g., 24.5 = 24.5%)\n"
        )
        ...
        ttk.Label(out, text="Forward variance = (s2^2*T2 - s1^2*T1) / (T2 - T1)").grid(row=4, column=0, columnspan=2, sticky="w")
        self.fwd_var_val = ttk.Label(out, text="-")
        self.fwd_var_val.grid(row=4, column=2, columnspan=2, sticky="w", padx=(8, 0))

        ttk.Label(out, text="Forward volatility s_fwd (annualized)").grid(row=5, column=0, columnspan=2, sticky="w", pady=(6, 0))
        self.fwd_vol_val = ttk.Label(out, text="-")
        self.fwd_vol_val.grid(row=5, column=2, columnspan=2, sticky="w", padx=(8, 0), pady=(6, 0))

        ttk.Label(out, text="Forward Factor FF = (s1 - s_fwd) / s_fwd").grid(row=6, column=0, columnspan=2, sticky="w", pady=(6, 0))
    ...
    def _parse_inputs(self):
        ...
        if dte2 <= dte1:
            raise ValueError("Require DTE2 > DTE1 so that T2 > T1.")
        if iv1 < 0 or iv2 < 0:
    ...
    def compute(self):
        ...
        denom = T2 - T1
        if denom <= 0:
            messagebox.showerror("Invalid maturities", "T2 must be greater than T1.")
            return
        ...
        if fwd_var < 0:
            ...
            messagebox.showwarning(
                ...
                "Computed forward variance is negative.",
            )
            return
        ...
        if ff_ratio is None:
            self.ff_ratio_val.config(text="undefined (s_fwd = 0)")
            self.ff_pct_val.config(text="n/a")
        else:
    ...
    def clear(self):
        for var in (self.dte1_var, self.iv1_var, self.dte2_var, self.iv2_var):
            var.set("")
        for lbl in (
            self.T1_val, self.T2_val, self.s1_val, self.s2_val,
            self.tv1_val, self.tv2_val, self.fwd_var_val,
            self.fwd_vol_val, self.ff_ratio_val, self.ff_pct_val,
        ):
            lbl.config(text="-")
